...
//	private static class binarySearch implements Comparator<Fruit3> {
//		public int compare(Fruit3 cc, Fruit3 cc1) {
//			 if(cc.name.compareTo(cc1.name) > 0) {
//				 return 1;
//			 } else if(cc.name.compareTo(cc1.name) < 0) {
//				 return -1;
//			 } else {
//				 return 0;
//			 }			 			
...
public class 실습3_7객체배열이진탐색 {
	...
	static int binarySearch(Fruit3[]a, Fruit3 f, Comparator<Fruit3> cc) {
		...
		do {
		    ...
		    if (cc.compare(a[pc], f) > 0) {
		        pr = pc - 1;  // 수정: pl = pc + 1에서 pr = pc - 1로 변경
		    } else if (cc.compare(a[pc], f) < 0) {
		        pl = pc + 1;  // 수정: pr = pc - 1에서 pl = pc + 1로 변경
		    }
		} while (pl <= pr);
		return -1;
	}
